<template>
  <div class="review bg-white pt-[19px] pr-[25.5px] pb-[20px] pl-[20px]">
    <div class="flex justify-between mb-[8px]">
      <div class="flex">
        <img
          class="w-[42px] h-[42px] rounded-50 mr-[10px]"
          :src="review.img"
          alt="person"
        />

        <div>
          <p class="font-500-13">{{ review.name }}</p>
          <p class="font-norm-12 text-[#5a7180]">
            {{ review.profession }} | {{ review.time }} ago
          </p>
        </div>
      </div>
    ...
    <p class="font-norm-12 text-[#5a7180] pr-[30px]">
      {{review.text}}
    </p>
  </div>
</template>
